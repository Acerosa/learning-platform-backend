...
select is((select count(*)::int from learning.questions q join learning.activity_versions v on v.id = q.activity_version_id join learning.activities a on a.id = v.activity_id join learning.modules m on m.id = a.module_id where m.stable_key = 'unit-3-cyber-security'), 1147, 'Unit 3 question rows include Batch A1 banks plus Batch B marking versions');
