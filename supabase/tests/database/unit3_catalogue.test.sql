...
select is((select count(*)::int from learning.questions q join learning.activity_versions v on v.id = q.activity_version_id join learning.activities a on a.id = v.activity_id join learning.modules m on m.id = a.module_id where m.stable_key = 'unit-3-cyber-security'), 592, 'Unit 3 has 592 importable grounded question records');
