-- Allow an already-linked learner to join an additional open teaching group.
-- Previously, complete_learner_onboarding treated "profile exists, no enrolment
-- in this group" as ONBOARDING_CONFLICT, which blocked normal late joins.
...
comment on function api.complete_learner_onboarding(text, text, text, text) is
  'Atomically creates or safely links the current Auth learner and enrols them in one open registration group. Existing linked learners may join an additional open group without recreating their account.';
