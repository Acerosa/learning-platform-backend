-- Accept Core evidence-only submissions without client-authored scores.
-- Client-marked Unit 3 / T Level payloads remain valid. Evidence-only items
-- are marked on the server from protected learning.question_marking rows.

create table learning.question_marking (
  question_id uuid primary key
    references learning.questions (id) on delete cascade,
  spec jsonb not null,
  created_at timestamptz not null default now(),
  constraint question_marking_spec_object
    check (jsonb_typeof(spec) = 'object')
);

comment on table learning.question_marking is
  'Protected formative marking specifications. Never granted to authenticated learners.';

create function learning.reject_published_question_marking_change()
returns trigger
language plpgsql
set search_path = ''
as $$
...
create trigger question_marking_immutable_after_publication
before insert or update or delete on learning.question_marking
for each row execute function learning.reject_published_question_marking_change();

alter table learning.question_marking enable row level security;

revoke all on table learning.question_marking from public, anon, authenticated;
revoke all on function learning.reject_published_question_marking_change()
  from public, anon, authenticated;

create function learning.mark_evidence_response(
  p_question_id uuid,
  p_payload jsonb,
  p_max_score numeric
)
...
begin
  ...
  if v_mode = 'single-choice' then
    v_ok := coalesce(p_payload ->> 'optionId', '') = coalesce(v_spec ->> 'correctOptionId', '');
  ...
  if v_mode = 'classification' then
    v_ok := coalesce(p_payload ->> 'categoryId', '') = coalesce(v_spec ->> 'correctCategoryId', '');
    awarded_score := case when v_ok then p_max_score else 0 end;
    is_correct := v_ok;
    requires_review := false;
    return next;
...
begin
  ...
  loop
    ...
    v_item_source := case when v_has_score then 'client' else 'server' end;
    ...
    if v_item_source = 'client' then
      ...
      v_requires_review := not v_is_correct;
    else
      select mark.awarded_score, mark.is_correct, mark.requires_review
      into v_awarded_score, v_is_correct, v_requires_review
...
comment on function api.submit_attempt(
  ...
  'Stores an idempotent learner attempt. Client-marked items keep awarded_score/is_correct; Core evidence-only items are marked on the server. Identity is always auth.uid().';
