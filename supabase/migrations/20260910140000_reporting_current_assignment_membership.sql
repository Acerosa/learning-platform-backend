-- Phase 1A.1: authoritative current activity membership for reporting.
--
-- Problem: multiple activity_versions of the same logical activity key can be
-- active for one group. That is legitimate for version-pinned learner APIs
-- (submit_attempt, mark_formative_response, save_activity_state), which resolve
-- by exact activity_version. Reporting must not treat every active version as
-- current required work.
--
-- Authoritative reporting rule (per learner hub scope):
--   one group + one activity_id
--   → learning.current_activity_assignment_id among active assignments
--   → one reporting row
--
-- Selection order (not bare semver):
--   1. prefer non-retired activity_versions
--   2. then latest activity_versions.published_at
--   3. then activity_versions.version text (tie-break)
--   4. then assignment id
--
-- This migration does NOT deactivate historical assignments. Doing so breaks
-- version-pinned submissions still targeting older versions (proven by
-- activity_state dual-version fixtures and Cyber catalogue 1.0.0 vs 1.1.0).
-- Assignment-level uniqueness remains a separate design decision.
--
-- Hosted apply is deferred until separate review. Do not supabase db push.

create or replace function learning.current_activity_assignment_id(
  p_group_id uuid,
  p_activity_id uuid
)
returns uuid
...
as $$
  ...
  where assignment.group_id = p_group_id
    ...
    and activity_version.activity_id = p_activity_id
  order by
    (activity_version.retired_at is null) desc,
    activity_version.published_at desc nulls last,
    activity_version.version desc,
    assignment.id desc
  limit 1;
$$;

comment on function learning.current_activity_assignment_id(uuid, uuid) is
  'Authoritative current active assignment for one group + one activity for reporting membership. Prefers non-retired, then latest published_at, then version text, then assignment id. Does not mutate assignments.';

revoke all on function learning.current_activity_assignment_id(uuid, uuid)
  from public, anon, authenticated;

create or replace function api.my_hub_activity_progress(p_hub_code text)
returns table (
  hub_code text,
  activity_key text,
  activity_title text,
...
begin
  ...
  left join lateral (
    ...
    where delivery.activity_version_id = hub_assignment.activity_version_id
      and delivery.active
      and (
        delivery.group_id is null
        or delivery.group_id = hub_assignment.group_id
      )
    order by
      case when delivery.group_id = hub_assignment.group_id then 0 else 1 end,
      delivery.sort_order nulls last,
      delivery.id
    limit 1
...
comment on function api.my_hub_activity_progress(text) is
  'Phase 1A/1A.1 learner reporting: hub-scoped completed-attempt progress for the authenticated learner. One row per authoritative current assignment among active group membership (non-retired, latest published_at). Identity is auth.uid() via learning.current_student_id(). Does not deactivate historical assignments. Excludes formative_checks and activity_states. Does not accept learner identifiers. Does not expose response_payload.';
