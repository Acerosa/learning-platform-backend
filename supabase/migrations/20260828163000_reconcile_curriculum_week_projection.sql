-- Reconcile curriculum week catalogue rows by teaching slot when package week keys change.
-- Fixes production 23505 on curriculum_weeks_module_number_unique during T Level publication.

-- One-time: align legacy T Level foundations week key with current week-1 package identity.
update learning.curriculum_weeks as week
set
  stable_key = 'week-1',
  updated_at = clock_timestamp()
from learning.modules as module
join learning.courses as course on course.id = module.course_id
where week.module_id = module.id
  and module.stable_key = 'tlevel-software-development'
  and course.stable_key = 't-level-digital-software-development'
  and week.stable_key = 'foundations'
  and week.week_number = 1
  and not exists (
    select 1
    from learning.curriculum_weeks as existing
    where existing.module_id = week.module_id
      and existing.stable_key = 'week-1'
      and existing.id <> week.id
  );
