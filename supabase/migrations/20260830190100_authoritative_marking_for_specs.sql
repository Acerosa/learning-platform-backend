-- Prefer protected marking specs over client-supplied scores.
...
comment on function api.submit_attempt(
  ...
  'Stores an idempotent learner attempt. Identity is always auth.uid(). Client awarded_score/is_correct are ignored when a protected marking spec exists; remaining client-marked items are limited to questions without a spec.';
